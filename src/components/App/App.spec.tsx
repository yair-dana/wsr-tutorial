...
describe('App', () => {
  ...
  describe('Role Details', () => {
    it('should display `Edit` button when click `Save` and when click again `Edit', async () => {
      expect(await driver.get.editSaveButtonTitle()).toBe('Edit');
      await driver.when.editSaveRoleButtonClick();
      expect(await driver.get.editSaveButtonTitle()).toBe('Save');
      await driver.when.editSaveRoleButtonClick();
      expect(await driver.get.editSaveButtonTitle()).toBe('Edit');
    });

    it('should be able to change role details and save', async () => {
      expect(await driver.get.editSaveButtonTitle()).toBe('Edit');

      await driver.when.editSaveRoleButtonClick();
      await driver.when.enterExperience('NEW EXPERIENCE');
      await driver.when.enterOfficialTitle('NEW OFFICIAL TITLE');
      await driver.when.editSaveRoleButtonClick();

      expect(await driver.get.experienceValue()).toBe('NEW EXPERIENCE');
      expect(await driver.get.officialValue()).toBe('NEW OFFICIAL TITLE');
    });
  });
});
